...
class PlanningGraph():
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for e in node_a1.action.effect_add:
            for p in node_a2.action.precond_neg:
                if e == p:
                    return True
                
        for e in node_a1.action.effect_rem:
            for p in node_a2.action.precond_pos:
                if e == p:
                    return True
                
        for e in node_a2.action.effect_add:
            for p in node_a1.action.precond_neg:
                if e == p:
                    return True
                
        for e in node_a2.action.effect_rem:
            for p in node_a1.action.precond_pos:
                if e == p:
                    return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for n_a1 in node_a1.parents:
            for n_a2 in node_a2.parents:
                if n_a1.is_mutex(n_a2):
                    return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        for n_s1 in node_s1.parents:
            for n_s2 in node_s2.parents:
                if not n_s1.is_mutex(n_s2):
                    # return false if the pair is not mutex
                    return False
        return True
    ...
    def h_levelsum(self) -> int:
        ...
        for goal in goals:
            if goal not in self.s_levels[-1]:
